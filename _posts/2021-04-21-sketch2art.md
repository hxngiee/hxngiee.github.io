...
```python
 with torch.no_grad():
        ...
        style_image = Variable(transform(Image.open('inputs/'+str(style_number)+'.jpg').convert('RGB')).unsqueeze(0).cuda()) if style_number != 10 else None

        opts.num_style = 1 if style_number != 10 else 10
...
```python
    ...
    return buf.getvalue()
```
4. 응답 JSON 형태이며 다음과 같은 예측 결과 포함{"class_id":'123', 'clas_name':'123'}
