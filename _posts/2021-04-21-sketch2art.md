...
그 외 특이한 사항으로는 흔히 Image Synthesis에 많이 사용되는 Instance Normalization 대신 Layer Normalization이 사용됐습니다. Instance Normalization의 경우 채널 각각을 normalize하여 채널간 유기적인 정보를 파괴돼 적절한 스타일이 반영되지 않은 결과를 나타냈습니다. 따라서 채널을 같이 Normalize하는 Layer Normalization을 사용함으로써 채널간 유기적인 관계를 유지하고 Content에 대해서 채널축마다 특정 스타일을 성공적으로 합성시킬 수 있었습니다.

### PONO
Positional Normalization 기법을 통해 기존 baseline model의 성능을 향상시켰습니다. Pixel position별 Feature 차원의 정규화를 수행하는 PONO는 네트워크가 Structural한 정보를 쉽게 파악하도록 도움으로써 GAN에 적용했을 때 Image Generation 성능을 크게 향상시키는 것으로 보고되었습니다. 해당 기법에 대한 자세한 설명은 [Positional Normalization 포스트](https://hxngiee.github.io/2021-05-07-PositionalNormalization/)에 작성하였습니다.
