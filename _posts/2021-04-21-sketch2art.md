...
- GAN  
 ...
 Sketch를 Painting으로 바꿔주는 Image Translation 모델은 이미지 생성 방식에 따라 두 분류로 나눌 수 있습니다. Style Transfer를 이용한 방식과 GAN을 이용한 방식이 그 예입니다. Style Transfer의 경우, pre-train 네트워크를 이용한 **영상 최적화** 방식으로 새롭게 합성될 영상의 feature map이 Content/Style Image로부터 나온 feature map과 비슷한 특성을 갖도록 근사합니다. 반면, GAN의 경우 특정 Domain **이미지 데이터의 확률 분포를 근사**하는 모델 학습 방식으로 특정 Domain의 데이터를 다른 Domain의 데이터로 변환하는 Mapping Function을 학습합니다. 두 방법은 이미지 자체에서 optimization을 진행하는지 혹은 Domain별 데이터의 확률분포를 바탕으로 optimization 진행하는지에 따라 차이를 보입니다
...
**[3] [Style Transfer](https://blog.lunit.io/2017/04/27/style-transfer)**  
**[4] [AI Production Roadmap - AI 시스템의 요구사항과 기술의 방향](https://www.youtube.com/watch?v=ZfLdURY0qCs)**  
