...
## Model Training
### MUNIT
사용자가 하나의 Input Sketch에 대해 다양한 Stylized 결과물을 선택할 수 있도록 `Multi-Modal image translation` 방식의 [MUNIT](https://arxiv.org/abs/1804.04732)을 baseline 모델로 사용하였습니다. 기존의 Deterministic 모델의 한계을 넘어선 MUNIT은 이미지의 representation을 content space와 style space로 분해하여 한 One-to-One mapping을 지원합니다. 이때 content space(domain-invariant)는 서로 다른 도메인 사이에서 공유되고, style space(domain-specific)는 각 도메인마다 존재하며, source domain의 content vector와 target domain의 style vector의 조합을 통해 원하는 target image를 생산합니다. style distribution은 가우시안 분포를 띄는 vector로 간주하여 random sampling을 통해서도, user의 example-based image를 통해서도 추출이 가능합니다 
...
#### Auto-encoding
- Two domains come from the same distribution of underlying structures(content) but in different distributions of renderings(style)  
- A content space C that is shared, domain-invariant  
- Two style space S_1, S_2 that are unshared, domain-specific  
- During translation, the content should be preserved and the style should be modified  
- Diversity should come from the style distribution of the target domain  
- content captures the shared information between the two domains which is probably the head pose of the animal
- style space -> dog has different distribution of texture or colors or some small shape changes 

#### Translation
- translate an input image to another domain, we extract its content code
- sample different style codes from the target style distribution
- by sampling different style codes and recombining them with the input content
- we can generate diverse set of outputs that have different style
...
**[10] [PYTORCH 모델을 ONNX으로 변환하고 ONNX 런타임에서 실행하기](https://tutorials.pytorch.kr/advanced/super_resolution_with_onnxruntime.html)**  
**[11] [AI Production Roadmap - AI 시스템의 요구사항과 기술의 방향](https://www.youtube.com/watch?v=ZfLdURY0qCs)**  
