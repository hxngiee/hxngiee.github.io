...
## Model Training
### MUNIT
사용자가 하나의 Input Sketch에 대해 다양한 Stylized 결과물을 선택할 수 있도록 `Multi-Modal` image translation 방식의 [MUNIT](https://arxiv.org/pdf/1804.04732.pdf)을 baseline 모델로 적용하였습니다. 기존의 Deterministic한 One-to-One mapping의 제약을 넘어선 MUNIT은 이미지의 representation을 content space와 style space로 분해합니다. 이때 content space는 서로 다른 도메인 사이에서 공유되고, style space는 각 도메인마다 존재하며, source domain의 content vector와 target domain의 style vector의 조합을 통해 원하는 형태의 target image로 재해석합니다. style distribution은 가우시안 분포를 띄는 vector로 간주하여 random sampling을 통해서도, user의 example-based image를 통해서도 추출이 가능합니다 
